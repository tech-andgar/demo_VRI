...
function initializeFirebaseAndMessaging() {
  ...
  try {
    ...
    if (firebase.messaging.isSupported()) {
      ...
      messagingInstance.onBackgroundMessage((payload) => {
        ...
        if (self.registration && typeof self.registration.showNotification === 'function') {
          self.registration
            ...
            .then(() =>
              console.log('[firebase-messaging-sw.js] Background FCM notification shown.'),
            )
            .catch((err) =>
              console.error(
                '[firebase-messaging-sw.js] Error showing background FCM notification:',
                err,
              ),
            );
        } else {
          console.error(
            '[firebase-messaging-sw.js] self.registration.showNotification is not available to display the FCM notification.',
          );
        }
      });
      ...
      console.warn(
        '[firebase-messaging-sw.js] Firebase Messaging is NOT supported in this Service Worker context.',
      );
      return false;
    }
  } catch (error) {
    console.error(
      '[firebase-messaging-sw.js] CRITICAL ERROR during Firebase initialization or Messaging setup:',
      error,
    );
    console.error(
      '[firebase-messaging-sw.js] Ensure firebaseConfig is correctly set with your REAL project values.',
    );
...
self.addEventListener('activate', (event) => {
  console.log(
    '[firebase-messaging-sw.js] Event: activate. Claiming clients and initializing Firebase.',
  );
  ...
  if (event.data && event.data.type === 'SHOW_SW_NOTIFICATION_TEST') {
    ...
    if (self.registration && typeof self.registration.showNotification === 'function') {
      self.registration
        ...
        .then(() =>
          console.log('[firebase-messaging-sw.js] Test notification shown via postMessage.'),
        )
        .catch((err) =>
          console.error(
            '[firebase-messaging-sw.js] Error showing test notification via postMessage:',
            err,
          ),
        );
    } else {
      console.error(
        '[firebase-messaging-sw.js] self.registration.showNotification not available for test message.',
      );
    }
  }
});
...
self.addEventListener('push', (event) => {
  ...
  if (!firebaseInitialized || !messagingInstance) {
    console.warn(
      '[firebase-messaging-sw.js] Push event received, but Firebase Messaging not initialized. Attempting init...',
    );
    if (!initializeFirebaseAndMessaging()) {
      console.error(
        '[firebase-messaging-sw.js] Failed to initialize Firebase for push event. Notification might not be displayed by FCM handlers.',
      );
  ...
  console.log(
    '[firebase-messaging-sw.js] Push event received, FCM should handle it via onBackgroundMessage if payload is FCM compliant.',
  );
});

console.log(
  '[firebase-messaging-sw.js] Service Worker script fully parsed and listeners attached.',
);
